var xx, slim, vpos, linespace, scale_x, swidth, spos;
...
if (active == true)
{
    ...
    if (global.language == "ja")
        xx -= 30
    draw_text_transformed((xx + (sin(siner / 4))), (y + (cos(siner / 4))), scr_gettext("mett_ratings", string(global.ratings)), (2 - (sin(siner / 4)) * 0.05), (2 - (cos(siner / 4)) * 0.05), 0)
    for (i = 0; i < 6; i += 1)
    {
        if (rq_v[i] >= 0)
        {
        ...
        scale_x = 1
        swidth = string_width(rq[i])
        if (swidth > (vpos - slim))
            scale_x = (vpos - slim) / swidth
        spos = round(vpos - swidth * scale_x)
        xx = 0
        if (rq_s[i] < 10)
